...
public class InMemoryUserStorage implements UserStorage {
    ...
    public User create(User user) {
        ...
        user.setId(userId++);
        log.info(
                "Добавлен пользователь с логином {}.", user.getLogin()
        );
    ...
    public User update(User user) {
        if (!getUsers().containsKey(user.getId())) {
            throw new ObjectNotFoundException(
                    "Пользователя не существует," + " необходима регистрация нового пользователя."
            );
        }
        validate(user);
        users.put(user.getId(), user);
        log.info(
                "Информация о пользователе {} обновлена.", user.getLogin()
        );
        return user;
    }
